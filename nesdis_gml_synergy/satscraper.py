...
import nesdis_gml_synergy.satlab as _satlab
...
def find_sites_on_grid(lon_lat_grid, sites, discard_outsid_grid = 2.2, 
...
def make_workplan(list_of_files = None,
                  ...
                  path2tmp_fld = '/mnt/telg/tmp/class_tmp/', 
    ...
    """
    if not isinstance(path2tmp_fld, type(None)):
        path2tmp_fld = _pl.Path(path2tmp_fld)
    ...
    if file_location == 'local':
        if file_processing_state == 'intermediate':
            ...
            if isinstance(list_of_files, type(None)):
                df = _pd.DataFrame(list(path2tmp_fld.glob('*.nc')), columns=['path2tempfile'])
            else:
                assert(False), 'programming required'
    ...
    elif file_location == 'ftp':
        df = _pd.DataFrame(list_of_files, columns=['fname_on_ftp'])
        df['path2tempfile'] = df.apply(lambda row: path2tmp_fld.joinpath(row.fname_on_ftp), axis = 1)
        df['path2intermediate_file'] =df.apply(lambda row: path2interfld.joinpath(row.fname_on_ftp), axis = 1)
    else:
        assert(False), f'{file_location} is not an option for file_location ... valid: (local, ftp)'
    # return df
...
def scrape_class(sites,
                 # path2local_files = None,
                 file_location='ftp',
                 # file_processing_state='raw',
    ...
    if no_of_cpu == 1:    
        for idx, row in workplan.iterrows():
            ds_at_sites = process_workplan_row(row, ftp, sites)
        ...
        # out['pool_return'] = pool.map(_partial(process_workplan_row, **{'ftp': ftp, 'sites': sites}), rows)
        ...
        # pool.map(partial(process_workplan_row, **{'ftp_settings': ftp_settings, 'sites': sites}), rows)
...
def concat_scrapefiles(path2interfld = '/mnt/telg/tmp/class_tmp_inter',
                     path2resultfld = '/mnt/telg/projects/GOES_R_ABI/data/sattelite_at_gml',
                     test = False):
