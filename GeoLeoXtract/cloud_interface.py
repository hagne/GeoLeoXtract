...
import GeoLeoXtract.info as ngsinfo
...
class AwsQueryHRRR(object):
    def __init__(self,
                 path2folder_local = '~/tmp/hrrr_scraper/individual/',
                 path2raw = '~/tmp/hrrr_scraper/raw/',
                 # satellite = '16',               
                 # sensor = 'VIIRS',
                 # product = 'ABI-L2-AOD',
                 # scan_sector = 'C',
        ...
        forcast_intervals: array-like
            Which intervals to consider. Default is None, which equals all 
            intervals up to 18 which equals range(19).Most files will have 19
            forcast hours (use range(19)). Some have up to 36 hours 
            (use range(37)). Or just specify exactly which you want,
            e.g. [1,5,10].
        ...
        if isinstance(forcast_intervals, type(None)):
            forcast_intervals = range(19)
            
        self.forcast_intervals= forcast_intervals
        self.aws_path = _pl.Path(aws_path)
        self.domain = domain
        ...
        self.path2data = _pl.Path(path2folder_local)
        self.path2data_tmp = _pl.Path(path2raw)
        self.overwrite = overwrite
    ...
    def estimate_disk_usage(self, sample_size = 10): #mega bites
        ...
        #     sizes = _pd.DataFrame(p2faws, columns = ['fn'])[::10].apply(lambda row: self.aws.disk_usage(row.fn), axis = 1)
        #     disk_space_needed = sizes.mean() * p2faws.shape[0]
        sizes = self.workplan.iloc[::step_size].apply(lambda row: self.aws.disk_usage(row.files_on_ftp), axis = 1)
        # sizes = self.workplan.iloc[::int(self.workplan.shape[0]/sample_size)].apply(lambda row: self.aws.disk_usage(row.path2file_aws), axis = 1)
        disk_space_needed = sizes.mean() * self.workplan.shape[0]
        
        # get remaining disk space after download
        du = _psutil.disk_usage(self.path2data)
        disk_space_free_after_download = 100 - (100* (du.used + disk_space_needed)/du.total )
        out = {}
        out['disk_space_needed'] = disk_space_needed
        out['disk_space_free_after_download'] = disk_space_free_after_download
    ...
    def alltasks(self):
        if isinstance(self._alltasks, type(None)):
            ...
            def fn2datetime(row):
            #     try:
                day = _pd.to_datetime(row.files_on_ftp.parent.parent.name.split('.')[-1])
                hour = _pd.to_timedelta(int(row.files_on_ftp.name.split('.')[1][1:-1]), 'hour')
                out = day + hour
            #     except ValueError:
            #         out = np.nan
                return out
            ...
            df['path'] = df.apply(lambda row: self.aws_path.joinpath(f'hrrr.{row.name.year}{row.name.month:02d}{row.name.day:02d}').joinpath(self.domain).joinpath('*'), axis= 1)
            ...
            for idx,row in df.iterrows():
                ...
                fat = [_pl.Path(f) for f in fat if (self.configuration in f) and (f[-6:] == '.grib2')]
            ...
            workplan = _pd.DataFrame(list_of_files_onftp, columns = ['files_on_ftp'])
            ...
            workplan = workplan[workplan.forcast_interval.isin(self.forcast_intervals)]
            
            ### generate temporary output path this is the path where the raw files are stored
            
            def row2raw_name_local(row):
                dt = row.cycle_datetime
                dts = f'{dt.year:04d}{dt.month:02d}{dt.day:02d}'
                uwe = row.files_on_ftp.name.split('.')
                uwe.insert(3, dts)
                return '.'.join(uwe)
            
            workplan['path2tempfile'] = workplan.apply(lambda row: path2data_tmp.joinpath(row2raw_name_local(row)), axis=1)
    ...
    def download(self, test = False, overwrite = False,
                 # alternative_workplan = False,
                 error_if_low_disk_space = True):
        """
        ...
        for idx, row in workplan.iterrows():
            if not overwrite:
                if row.path2tempfile.is_file():
                    continue
            #### TODO memory leak ... this did not help ... the following line is trying to deal with it. Its actaully not clear if the leak only happens when processing ... then the following line should not help
            # self.aws.clear_instance_cache() 
            # next try: reload aws instance: not helping
            # self.aws = _s3fs.S3FileSystem(anon=True, skip_instance_cache=True)
            out = self.aws.get(row.files_on_ftp.as_posix(), row.path2tempfile.as_posix())
            if test:
                break
        return out
