...
class GeosSatteliteProducts(object):
    def __init__(self,file, product_version = None, verbose = False):
        ...
        self.shape = None # allows the user to attache a shape file to instance, usefull for project_on_shape
        ...
        if not isinstance(product_version, type(None)):
            self.product_info['version'] = product_version
...
#         self._varname4test = 'CMI_C02'
    ...
    def lonlat(self):
        if isinstance(self._lonlat, type(None)):
            ...
            if (self.grid_type == 'scan_angle') :
                assert('lat' not in self.ds.coords), 'This should not exist, this would mean there are both x,y and lat, lon'
                
                # The following did not work, as the masking also maskes the lat lon, which resulst in nan coordinates
                #if 'lat' in self.ds.variables: # this can be the case if this is a subsetted (to a shape) GeosSatteliteProducts instance, so no need to calculate it again.
    ...
    def project_on_shape(self, shape):
        ...
        Parameters
        -----------
        shape: geopandas.geodataframe.GeoDataFrame,pathlib.Path, or str (path)
            A GeoDataFrame with the shape geometry, or a path (str or obj) to the 
            shape file that then will be opended with geopandas.

        Returns
        --------
        GeosSatteliteProducts instance where the data in ds is removed to inside 
        the shape.
        """
        #### open/check shape    
        ...
        isinstance(shape, _gpd.geodataframe.GeoDataFrame)

        #### create a GeoDataFrame from lat and lon in order to get an intercept of two geodataframes
        lon, lat = self.lonlat
        points = [shapely.geometry.Point(lo, la) for lo, la in zip(lon.flat, lat.flat)]
        points_gdf = _gpd.GeoDataFrame(geometry=points)
        
        # Ensure that both GeoDataFrames use the same CRS
        points_gdf.set_crs(shape.crs, inplace=True)
        
        # Perform spatial join
        inside_points = _gpd.sjoin(points_gdf, shape, how="inner", op='intersects')
        
        # Initialize mask with False (indicating data points are outside the shape)
        mask = _np.zeros_like(lat, dtype=bool)
        
        # Set the mask to True for points inside the shape
        inside_indices = [index for index in inside_points.index]
        mask.flat[inside_indices] = True

        # apply the mask to the dataset
        mask_da = _xr.DataArray(mask, coords=[self.ds.y, self.ds.x], dims=["y", "x"])
        dssub = self.ds.where(mask_da, 
                      drop = True,
                     )
        
        shape_projection = GeosSatteliteProducts(dssub, 
        ...
        **pcolor_kwargs : TYPE
        ...
        if backend == 'basemap':
            if isinstance(ax, type(None)):
                ax = _Basemap(resolution='c', projection='aea', area_thresh=5000, 
            ...
            pc = ax.pcolormesh(lons, lats, ds[variable], latlon=True, **pcolor_kwargs)
            ...
            if show_features:
                ...
                ax.add_feature(cartopy.feature.STATES)  # Add states

            if not isinstance(shape, type(None)):
                shape.plot(ax=ax, color='none', edgecolor='black', transform=projection)
...
class Grid2SiteProjection(object):
    ...
    def projection2point(self):
        if isinstance(self._projection2poin, type(None)):
            ...
            self.tp_ds = ds
            self.tp_coord1 = coord1
            self.tp_coord2 = coord2
            
            
            for e,(idx, rowsmt) in enumerate(self.closest_grid_points.iterrows()):                
                
                # I think there is a bug in xarray '2025.1.1', but the following code stoped working
                # isel_dict = {coord1: int(rowsmt.argmin_y), coord2: int(rowsmt.argmin_x)}
                # # self.tp_isel_dict = isel_dict
                # ds_at_site = ds[isel_dict]
                
                # instead select one after the other
                ds_at_site = ds.isel({coord1: int(rowsmt.argmin_y)}).isel({coord2: int(rowsmt.argmin_x)})
                
                #x and y seam to be interchanged
